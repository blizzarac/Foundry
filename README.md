...
Everything on this page — enemy stats and their tier-scaling formula, prologue floor tables, Foundry sector-generation coefficients (pack density, elite bump tiers, chest/terminal odds, key-drop odds, gate jump size, every Sector Key modifier), and the whole economy (shop upgrade costs and caps, restocks, orb prices, the prototype gamble, the key-fabrication curve, item and key salvage values) — lives as plain data in **`config.js`**, loaded before the game itself. Retuning any of it is a numbers edit, not a code edit.
...
What's *not* in there: item/affix/rarity tables (base types, prefix/suffix pools, uniques), node-event timers, and small combat constants like dash range — those still live in `rl.js`, alongside terrain generators, boss attack patterns, and AI, which are behavior rather than balance dials and always will be.
